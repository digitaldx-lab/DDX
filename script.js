// ==========================================================
// 1. Element Selection (Consolidated)
// ==========================================================
const chatBubble = document.getElementById("pal-bubble");
const chatWindow = document.getElementById("pal-chat");
const closeButton = document.getElementById("pal-close");
const sendButton = document.getElementById("pal-send");
const messageInput = document.getElementById("pal-user-input");
const messageContainer = document.getElementById("pal-messages");
const clearChatButton = document.getElementById("pal-clear-chat");

// Add this at the top of your chat script, after element selection
let cartSystem = null;
let availableTests = {};

// Key for sessionStorage
const PAL_CHAT_KEY = "palConversation";

// ==========================================================
// 2. Initialize Cart System and Test Data
// ==========================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const initCart = () => {
        if (window.labCart) {
            cartSystem = window.labCart;
            loadAvailableTests();
            console.log('Cart system initialized with tests:', Object.keys(availableTests).length);
        } else {
...
function loadAvailableTests() {
    ...
    if (window.availableTestsArray && Array.isArray(window.availableTestsArray)) {
        window.availableTestsArray.forEach(test => {
            availableTests[test.id] = test;
            availableTests[test.name.toLowerCase()] = test;
        });
        console.log('Loaded tests from availableTestsArray:', window.availableTestsArray.length);
    } else {
        ...
        console.log('Loaded default tests:', defaultTests.length);
    }
}

// ==========================================================
// 3. Chat UI Toggle Logic
// ==========================================================
...
function clearChat() {
    sessionStorage.removeItem(PAL_CHAT_KEY);
    messageContainer.innerHTML = '';
    
    setTimeout(() => {
        appendMessage("Hello! I'm PAL. How can I assist you with diagnostic tests today?", "bot", false, true);
    }, 200);
}

// ==========================================================
// 5. Message Sending and AI Logic (FIXED)
// ==========================================================

sendButton.addEventListener("click", sendToPal);
messageInput.addEventListener("keypress", (e) => {
    if (e.key === "Enter") sendToPal();
});

// The core function to send message to the backend
async function sendToPal() {
    const text = messageInput.value.trim(); 
    if (!text) return;

    // Check if this is a direct test addition request FIRST
    const testToAdd = detectTestRequest(text);
    if (testToAdd && cartSystem) {
        console.log('Direct test addition detected:', testToAdd.name);
        cartSystem.addItem(testToAdd);
        appendMessage(`✅ I've added "${testToAdd.name}" to your cart! Would you like to add another test or ask me anything else?`, "bot", false, true);
        messageInput.value = "";
        
        // Auto-open cart after adding
        setTimeout(() => {
            if (cartSystem && cartSystem.showCartAside) {
                cartSystem.showCartAside();
            }
        }, 500);
        return;
    }
    ...
    try {
        const messages = getPalConversation();
        messages.push({ role: "user", content: text });
        
        // API Call
        const res = await fetch("http://localhost:5000/api/pal", {
            method: "POST",
        ...
        removeLoading();
        if (data.reply) {
            // Process the response to add cart buttons for mentioned tests
            const processedReply = processBotResponse(data.reply);
            appendMessage(processedReply, "bot");
        } else {
            throw new Error("No reply in response");
        }
        
    } catch (err) {
        console.error("PAL API Error:", err);
        removeLoading();
        appendMessage("😔 Oops, PAL lost signal. Try again!", "bot", false, true);
    }
}

// ==========================================================
// 6. Cart Integration Functions (FIXED AND OPTIMIZED)
// ==========================================================

// Function to detect direct test requests
function detectTestRequest(text) {
    if (!text || !cartSystem) return null;
    
    const lowerText = text.toLowerCase().trim();
    console.log('Detecting test request for:', lowerText);
    
    // Simple direct matches first
    for (const key in availableTests) {
        const test = availableTests[key];
        if (typeof test === 'object') {
            const testNameLower = test.name.toLowerCase();
            // Check for direct matches like "add haemoglobin test"
            if (lowerText.includes('add ') && lowerText.includes(testNameLower)) {
                console.log('Direct match found:', test.name);
                return test;
            }
            // Check for "I want haemoglobin test"
            if ((lowerText.includes('want ') || lowerText.includes('need ') || lowerText.includes('book ')) && 
                lowerText.includes(testNameLower)) {
                console.log('Want/need match found:', test.name);
                return test;
            }
        }
    }
    
    // Common patterns for test requests
    const testPatterns = [
        /(?:add|book|schedule|order).*?(haemoglobin|blood test|urine test|lipid panel|glucose test|thyroid test|liver function|kidney function)/i,
        /(?:i want|i need|i'd like).*?(haemoglobin|blood test|urine test|lipid panel|glucose test|thyroid test)/i,
        /(?:test for|check for).*?(cholesterol|diabetes|thyroid|liver|kidney|anaemia|anemia)/i
    ];
    ...
    for (const pattern of testPatterns) {
        ...
        if (match) {
            ...
            console.log('Pattern match found:', testName);
            const test = findTestByName(testName);
            if (test) return test;
        }
    }
...
// Function to find test by name (improved)
function findTestByName(testName) {
    const searchName = testName.toLowerCase();
    
    // First, try exact match in availableTests
    if (availableTests[searchName]) {
        return availableTests[searchName];
    }
    
    // Then, search through all tests for partial matches
    for (const key in availableTests) {
        const test = availableTests[key];
        if (typeof test === 'object') {
            const testNameLower = test.name.toLowerCase();
            if (testNameLower.includes(searchName) || searchName.includes(testNameLower)) {
                return test;
            }
        }
    }
...
// Process bot response to add cart buttons for mentioned tests
function processBotResponse(response) {
    if (!response || !cartSystem) return response;
    
    let processedResponse = response;
    const testMentions = extractTestMentions(response);
    
    console.log('Found test mentions:', testMentions.length);
    
    testMentions.forEach(testInfo => {
        const test = findTestByName(testInfo.name);
        if (test) {
            const buttonHtml = `<br><button class="test-add-btn" data-test-id="${test.id}">
                <i class="fas fa-cart-plus"></i> Add "${test.name}" to Cart - ₦${test.price.toLocaleString()}
            </button>`;
            
            // Replace only the first occurrence to avoid duplicates
            processedResponse = processedResponse.replace(testInfo.original, `${testInfo.original}${buttonHtml}`);
        }
    });
...
// Extract test mentions from bot response
function extractTestMentions(text) {
    if (!text) return [];
    
    const testMentions = [];
    const testNames = [];
    
    // Get all test names from availableTests
    for (const key in availableTests) {
        const test = availableTests[key];
    ...
    // Look for test names in the text
    testNames.forEach(testName => {
        const regex = new RegExp(`\\b${testName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'gi');
        const matches = text.match(regex);
        if (matches) {
            matches.forEach(match => {
                // Avoid duplicates
                if (!testMentions.some(mention => mention.original === match)) {
                    testMentions.push({
                        original: match,
...
// Global function to add tests to cart (called from buttons)
window.addTestToCart = function(testId) {
    if (!cartSystem) {
        console.error('Cart system not available');
        appendMessage("❌ Cart system is not available. Please refresh the page.", "bot", false, true);
        return;
    }
    
    const test = availableTests[testId];
    if (test) {
        cartSystem.addItem(test);
        
        // Show confirmation in chat
        appendMessage(`✅ Added "${test.name}" to your cart!`, "bot", false, true);
        ...
        appendMessage("❌ Sorry, I couldn't find that test. Please try again.", "bot", false, true);
    }
};

// ==========================================================
// 7. Persistence and Message Utilities
// ==========================================================

function getPalConversation() {
    const chat = sessionStorage.getItem(PAL_CHAT_KEY); 
    return JSON.parse(chat || "[]");
}

function saveMessage(role, content) {
    const chat = getPalConversation();
    chat.push({ role, content });
    sessionStorage.setItem(PAL_CHAT_KEY, JSON.stringify(chat));
}

function removeLoading() {
    const messages = messageContainer.querySelectorAll(".pal-message.bot");
...
function loadExistingMessages() {
    const chatHistory = getPalConversation();
    messageContainer.innerHTML = '';
    
    chatHistory.forEach(msg => {
        const msgEl = document.createElement("div");
        msgEl.classList.add("pal-message", msg.role);
        
        // Check if message contains HTML (cart buttons)
        if (/<[a-z][\s\S]*>/i.test(msg.content)) {
            msgEl.innerHTML = msg.content;
            
            // Re-attach event listeners to cart buttons
            setTimeout(() => {
                const buttons = msgEl.querySelectorAll('.test-add-btn');
                buttons.forEach(button => {
                    const testId = button.getAttribute('data-test-id');
                    button.onclick = () => window.addTestToCart(testId);
                });
            }, 100);
        } else {
...
// ==========================================================
// 8. Enhanced Message Display with Markdown Support
// ==========================================================

async function appendMessage(content, sender, loading = false, skipTyping = false) {
    const msgContainer = messageContainer;
    const msg = document.createElement("div");
    msg.classList.add("pal-message", sender);
    ...
    // BOT message: Handle HTML content including buttons
    const hasHTML = /<[a-z][\s\S]*>/i.test(content);
    
    if (hasHTML) {
        // Content contains HTML (like our cart buttons)
        msg.innerHTML = content;
        msgContainer.appendChild(msg);
        
        // Re-attach event listeners to any buttons in the message
        setTimeout(() => {
            const buttons = msg.querySelectorAll('.test-add-btn');
            buttons.forEach(button => {
                const testId = button.getAttribute('data-test-id');
                button.onclick = () => window.addTestToCart(testId);
            });
        ...
        if (skipTyping) {
            ...
            for (const node of nodes) {
                if (node.nodeType === Node.TEXT_NODE) {
                    await typeText(inner, node.textContent, 10);
                } else {
